CREATE TABLE EventSP(
	...
	, FechaOperacion DATE
	, Exitoso INT --1 SÍ, 2 No
	, TipoEvento INT --1 Facturar
)
ALTER TABLE dbo.Propiedad
	ADD SaltoM3UltimaFactura INT
ALTER TABLE dbo.Propiedad
	ADD SaldoM3 INT 

GO CREATE PROCEDURE dbo.Calcular_Monto
			@inIdCC INT
			, @inIdPropiedad INT
			, @outMonto INT OUTPUT
AS
...
BEGIN TRY	
	...
	ELSE IF(@inIdCC = 3) --Impuesto sobre propiedad
		...
		WHERE cc.id = 3
		END
	ELSE IF(@inIdCC = 4) --Recolección de basura
		BEGIN 
...
GO
CREATE PROCEDURE dbo.sp_Facturar
...
BEGIN
	DECLARE @lo			   INT = 1
			...
			, @idFacLin	   INT		
BEGIN TRY
	DECLARE @Hoy INT = DAY(GETDATE())
			, @ultimoDiaMes INT = DAY(EOMONTH(GETDATE()))
	...
	IF (@Hoy = 28 --Febrero en año no bisiesto
	...
	ELSE IF (@Hoy = 29 --Febrero en año bisiesto
		...
		WHERE ( DAY(p.FechaRegistro ) IN (29
										  ...
										  , 31))
		END
	ELSE IF (@Hoy = 30 --Mes con 30 días
		AND @Hoy = @ultimoDiaMes) 
		BEGIN
		INSERT INTO @PropiedadesFacturan(idPropiedad)
		SELECT p.id
...
GO
CREATE PROCEDURE dbo.sp_Facturar_Corte_Agua
		@outResult INT OUTPUT
AS
BEGIN 
	DECLARE @lo INT = 1
...
BEGIN TRY
	...
	INSERT INTO @PropiedadCorteAgua(IdFactura
		...
		SELECT f.Id
				, f.Estatus
		FROM dbo.Facturas f		
		WHERE ( CAST(f.FechaCortaAgua AS DATE) = CAST(GETDATE() AS DATE)
			  AND (f.Estatus = 1)
			  )
